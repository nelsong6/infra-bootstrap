...
- **AKS** (`infra-aks`) — Free tier, 3× Standard_E2bs_v5 (2 vCPU, 16 GiB RAM each, 4 data disks each) with 128 GiB OS disks, Azure CNI Overlay, workload identity. Memory-optimized burstable family — workload is memory-heavy (idle session pods reserve RAM) but CPU-idle, so the 1:8 vCPU/RAM ratio fits better than B-family's 1:4. Was 3× B2s_v2 until session pods on tank-operator hit the per-node memory wall under normal usage.
